...
public extension Tx {
    ...
    func sigMsgV1(inIdx: Int, prevOuts: [Tx.Out], sigHashType originalSigHashType: SigHashType, extFlag: UInt8) -> Data {
        ...
        if sigHashType.isSingle {
            ...
            let shaSingleOutput = singleHash(outs[inIdx].data)
            outputData.append(shaSingleOutput)
        }
        
        return controlData + txData + inputData + outputData
    }
}
