...
final class BIP340Tests: XCTestCase {
    ...
    func testSignSchnorr() {
        ...
        for (secKey, pubKey, aux, message, signature) in signVectors {
            ...
            let newSignature = signSchnorr(message: messageData, secretKey: secKeyData, merkleRoot: .none, aux: auxData)
            XCTAssertEqual(newSignature, signatureData)
            // Verify those signatures for good measure.
            XCTAssert(verifySchnorr(signature: newSignature, message: messageData, pubKey: pubKeyData))
            ...
            for i in 0 ..< 10 {
                ...
                let altSignature = signSchnorr(message: messageData, secretKey: secKeyData, merkleRoot: merkleRoot, forceTweak: merkleRoot == .none, aux: auxRnd)
                print("merkleRoot: \(merkleRoot == .none ? "None": "Some")")
                let verificationResult = verifySchnorr(signature: altSignature, message: messageData, pubKey: tweakedKey)
    ...
    func testVerifySchnorr() {
        ...
        for ((pubKey, message, signature), result) in vectors {
            let pubKeyData = Data(hex: pubKey)
            let messageData = Data(hex: message)
            let signatureData = Data(hex:  signature)
            XCTAssertEqual(verifySchnorr(signature: signatureData, message: messageData, pubKey: pubKeyData), result)
        }
    }
}
