...
final class HashTests: XCTestCase {
    ...
    func testDoubleHash() {
        ...
        let previousOutputs = "fff7f7881a8099afa6940d42d1e7f6362bec38171ea3edf433541db4e4ad969f00000000ef51e1b804cc89d182d279655c3aa89e815b1b309fe287d9b2b55d57b90ec68a01000000"
        XCTAssertEqual(hash256(Data(hex: previousOutputs)).hex, "96b827c8483d4e9b96712b6713a7b68d6e8003a781feba36c31143470b4efd37")

    }

    func testHash160() {
        let publicKey = Data(hex: "025476c2e83188368da1ff3e292e7acafcdb3566bb0ad253f62fc70f07aeee6357")
        XCTAssertEqual(hash160(publicKey).hex, "1d0f172a0ecb48aee1be1f2687d2963ae33f71a1")
    }
    ...
    func testHashData() {
        ...
        let sigMsg = tx.segwitSignatureMessage(sighashType: .all, inputIndex: 1, scriptCode: Data(hex: "76a9141d0f172a0ecb48aee1be1f2687d2963ae33f71a188ac"), amount: 600_000_000)
        ...
        let secretKey = Data(hex: "619c335025c7f4012e556c2a58b2506e30b8511b53ade95ea316fd8c3286feb9")
        let sig = signECDSA(message: sighash, secretKey: secretKey)
    ...
    func testTxSignature() {
        ...
        var tx = Transaction(Data(hex: ""))
        
        let previousOutputs = [
            Transaction.Output(value: 625_000_000, script: .init(.init(hex: "2103c9f4836b9a4f77fc0d81f7bcb01b7f1b35916864b9476c241ce9fc198bd25432ac"))),
            Transaction.Output(value: Amount(0x0046c32300000000).byteSwapped, script: .init(.init(hex: "00141d0f172a0ecb48aee1be1f2687d2963ae33f71a1")))
        ]

        let secretKey1 = Data(hex: "619c335025c7f4012e556c2a58b2506e30b8511b53ade95ea316fd8c3286feb9")
        ...
        let secretKey0 = Data(hex: "bbc27228ddcb9209d7fd6f36b02f7dfa6252af40bb2f1cbc7a557da8027ff866")
