...
final class RegtestTests: XCTestCase {
    ...
    func testLegacy_pyBitcoin_all() {
        ...
        let prevOuts = [Transaction.Output(value: 0, script: Script([
            .dup,
            .hash160,
    ...
    func testLegacy_pyBitcoin2_all() {
        ...
        let sigMsg = tx.sigMsg(hashType: .all, inIdx: 0, subScript: .init(prevOuts[0].script)!)
    ...
    func testLegacy_pyBitcoin2_none() {
        ...
        let privKey0 = Data(hex: "81c70e36ffa5e3e6425dc19c7c35315d3d72dc60b79cb78fe009a335de29dd2201") // cRvyLwCPLU88jsyj94L7iJjQX5C2f8koG4G2gevN4BeSGcEvfKe9
        let pubKey0 = Data(hex: "03a2fef1829e0742b89c218c51898d9e7cb9d51201ba2bf9d9e9214ebb6af32708")
        let sigMsg = tx.sigMsg(hashType: .none, inIdx: 0, subScript: .init(prevOuts[0].script)!)
    ...
    func testLegacy_pyBitcoin2x2_all() {
        ...
        let prevOuts = [
            Transaction.Output(value: 0, script: Script([
                .dup,
                .hash160,
        ...
        let sigMsg0 = tx.sigMsg(hashType: .all, inIdx: 0, subScript: .init(prevOuts[0].script)!)
        ...
        let sigMsg1 = tx.sigMsg(hashType: .all, inIdx: 1, subScript: .init(prevOuts[1].script)!)
    ...
    func testLegacy_pyBitcoin2x2_none() {
        ...
        let sigMsg0 = tx.sigMsg(hashType: .none, inIdx: 0, subScript: .init(prevOuts[0].script)!)
        ...
        let sigMsg1 = tx.sigMsg(hashType: .none, inIdx: 1, subScript: .init(prevOuts[1].script)!)
    ...
    func testLegacy_1to1_all() {
        ...
        let prevOut0 = Transaction.Output(value: 0, script: .init(hex: "76a914786890276a55f3e6d2f403e3d595b6603964fa0d88ac"))
        ...
        let sigMsg = tx.sigMsg(hashType: .all, inIdx: 0, subScript: .init(prevOut0.script)!)
        ...
        let sighash = tx.sighash(.all, inIdx: 0, prevOut: prevOut0, scriptCode: .init(prevOut0.script)!, opIdx: 0)
        let sig = signECDSA(msg: sighash, privKey: privKey0)
        XCTAssertTrue(tx.checkSig(sig + HashType.all.data, pubKey: pubKey0, inIdx: 0, prevOut: prevOut0, script: .init(prevOut0.script)!, opIdx: 0))
        ...
        if  let coreOp = Script(coreTx.inputs[0].script.data)!.operations.first, case .pushBytes(let hashType) = coreOp {
            XCTAssertTrue(tx.checkSig(hashType, pubKey: pubKey0, inIdx: 0, prevOut: prevOut0, script: .init(prevOut0.script)!, opIdx: 0))
        } else {
    ...
    func testLegacy_1to1_allAny() {
        ...
        let prevOuts = [
            Transaction.Output(value: 0, script: .init(hex: "76a914786890276a55f3e6d2f403e3d595b6603964fa0d88ac"))
        ]
        let privKey0 = Data(hex: "828748ccadd3792f39841749da9618389dcce35ace39d94b131ae8d8a359804c") // 92aQLXE8yvQ1qHoXvPCSSoLP3AM65g98Pavxsb53MTdTv1BgKXE
        ...
        let sigMsg = tx.sigMsg(hashType: .allAnyCanPay, inIdx: 0, subScript: .init(prevOuts[0].script)!)
    ...
    func testLegacy_1to1_noneAny() {
        ...
        let prevOuts = [Transaction.Output(value: 0, script: .init(hex: "76a914786890276a55f3e6d2f403e3d595b6603964fa0d88ac"))]
        let privKey0 = Data(hex: "828748ccadd3792f39841749da9618389dcce35ace39d94b131ae8d8a359804c") // 92aQLXE8yvQ1qHoXvPCSSoLP3AM65g98Pavxsb53MTdTv1BgKXE
        let pubKey0 = Data(hex: "04ce88102d2af294198df851e4776e4c505e2f288cb253a244f69fb0ddc656f11e1286fb9309a39a92553e2ce3969eeb92ed30bd402a7cbc62ec7d7a4e32f7c125")
        
        let sigMsg = tx.sigMsg(hashType: .noneAnyCanPay, inIdx: 0, subScript: .init(prevOuts[0].script)!)
    ...
    func testLegacy_1to1_single() {
        ...
        let sigMsg = tx.sigMsg(hashType: .single, inIdx: 0, subScript: .init(prevOuts[0].script)!)
    ...
    func testLegacy_1to1_singleAny() {
        ...
        let prevOuts = [Transaction.Output(value: 312500000, script: .init(hex: "76a914786890276a55f3e6d2f403e3d595b6603964fa0d88ac"))]
        let privKey0 = Data(hex: "828748ccadd3792f39841749da9618389dcce35ace39d94b131ae8d8a359804c") // 92aQLXE8yvQ1qHoXvPCSSoLP3AM65g98Pavxsb53MTdTv1BgKXE
        let pubKey0 = Data(hex: "04ce88102d2af294198df851e4776e4c505e2f288cb253a244f69fb0ddc656f11e1286fb9309a39a92553e2ce3969eeb92ed30bd402a7cbc62ec7d7a4e32f7c125")
        
        let sigMsg = tx.sigMsg(hashType: .singleAnyCanPay, inIdx: 0, subScript: .init(prevOuts[0].script)!)
