...
final class RegtestTests: XCTestCase {
    ...
    func testLegacy_pyBitcoin_all() {
        ...
        let prevOuts = [Transaction.Output(value: 0, script: .init(ParsedScript([
            .dup,
            .hash160,
        ...
        tx.sign(privKeys: [privKey0], pubKeys: [pubKey0], sighashType: .all, inputIndex: 0, prevOuts: prevOuts)
    ...
    func testLegacy_pyBitcoin2_all() {
        ...
        let sigMsg = tx.signatureMessage(sighashType: .all, inputIndex: 0, scriptCode: prevOuts[0].script.data)
    ...
    func testLegacy_pyBitcoin2_none() {
        ...
        let privKey0 = Data(hex: "81c70e36ffa5e3e6425dc19c7c35315d3d72dc60b79cb78fe009a335de29dd2201") // cRvyLwCPLU88jsyj94L7iJjQX5C2f8koG4G2gevN4BeSGcEvfKe9
        let pubKey0 = Data(hex: "03a2fef1829e0742b89c218c51898d9e7cb9d51201ba2bf9d9e9214ebb6af32708")
        let sigMsg = tx.signatureMessage(sighashType: .none, inputIndex: 0, scriptCode: prevOuts[0].script.data)
        ...
        tx.sign(privKeys: [privKey0], pubKeys: [pubKey0], sighashType: SighashType.none, inputIndex: 0, prevOuts: prevOuts)
    ...
    func testLegacy_pyBitcoin2x2_all() {
        ...
        let sigMsg0 = tx.signatureMessage(sighashType: .all, inputIndex: 0, scriptCode: prevOuts[0].script.data)
        ...
        let sigMsg1 = tx.signatureMessage(sighashType: .all, inputIndex: 1, scriptCode: prevOuts[1].script.data)
    ...
    func testLegacy_pyBitcoin2x2_none() {
        ...
        let sigMsg0 = tx.signatureMessage(sighashType: .none, inputIndex: 0, scriptCode: prevOuts[0].script.data)
        ...
        let sigMsg1 = tx.signatureMessage(sighashType: .none, inputIndex: 1, scriptCode: prevOuts[1].script.data)
        ...
        tx.sign(privKeys: [privKey1], pubKeys: [pubKey1], sighashType: SighashType.none, inputIndex: 1, prevOuts: prevOuts)
    ...
    func testLegacy_1to1_all() {
        ...
        let prevOut0 = Transaction.Output(value: 0, script: .init(Data(hex: "76a914786890276a55f3e6d2f403e3d595b6603964fa0d88ac")))
        ...
        let sigMsg = tx.signatureMessage(sighashType: .all, inputIndex: 0, scriptCode: prevOut0.script.data)
        ...
        let sighash = tx.signatureHash(sighashType: .all, inputIndex: 0, previousOutput: prevOut0, scriptCode: prevOut0.script.data)
        let sig = signECDSA(msg: sighash, privKey: privKey0)
        XCTAssertTrue(tx.checkSignature(extendedSignature: sig + SighashType.all.data, publicKey: pubKey0, inputIndex: 0, previousOutput: prevOut0, scriptCode: prevOut0.script.data))
        ...
        if let coreOp = ParsedScript(coreTx.inputs[0].script.data)?.operations.first, case .pushBytes(let sighashType) = coreOp {
            XCTAssertTrue(tx.checkSignature(extendedSignature: sighashType, publicKey: pubKey0, inputIndex: 0, previousOutput: prevOut0, scriptCode: prevOut0.script.data))
        } else {
            XCTFail("Could not extract signature from transaction.")
        }

        //tx.sign(privKeys: [privKey0], pubKeys: [pubKey0], inputIndex: 0, prevOut: prevOut0, sighashType: .all)
    ...
    func testLegacy_1to1_allAny() {
        ...
        let sigMsg = tx.signatureMessage(sighashType: .allAnyCanPay, inputIndex: 0, scriptCode: prevOuts[0].script.data)
    ...
    func testLegacy_1to1_noneAny() {
        ...
        let prevOuts = [Transaction.Output(value: 0, script: .init(Data(hex: "76a914786890276a55f3e6d2f403e3d595b6603964fa0d88ac")))]
        let privKey0 = Data(hex: "828748ccadd3792f39841749da9618389dcce35ace39d94b131ae8d8a359804c") // 92aQLXE8yvQ1qHoXvPCSSoLP3AM65g98Pavxsb53MTdTv1BgKXE
        let pubKey0 = Data(hex: "04ce88102d2af294198df851e4776e4c505e2f288cb253a244f69fb0ddc656f11e1286fb9309a39a92553e2ce3969eeb92ed30bd402a7cbc62ec7d7a4e32f7c125")
        
        let sigMsg = tx.signatureMessage(sighashType: .noneAnyCanPay, inputIndex: 0, scriptCode: prevOuts[0].script.data)
        ...
        tx.sign(privKeys: [privKey0], pubKeys: [pubKey0], sighashType: .noneAnyCanPay, inputIndex: 0, prevOuts: prevOuts)
    ...
    func testLegacy_1to1_single() {
        ...
        let sigMsg = tx.signatureMessage(sighashType: .single, inputIndex: 0, scriptCode: prevOuts[0].script.data)
        ...
        tx.sign(privKeys: [privKey0], pubKeys: [pubKey0], sighashType: .single, inputIndex: 0, prevOuts: prevOuts)
    ...
    func testLegacy_1to1_singleAny() {
        ...
        let prevOuts = [Transaction.Output(value: 312500000, script: .init(Data(hex: "76a914786890276a55f3e6d2f403e3d595b6603964fa0d88ac")))]
        let privKey0 = Data(hex: "828748ccadd3792f39841749da9618389dcce35ace39d94b131ae8d8a359804c") // 92aQLXE8yvQ1qHoXvPCSSoLP3AM65g98Pavxsb53MTdTv1BgKXE
        let pubKey0 = Data(hex: "04ce88102d2af294198df851e4776e4c505e2f288cb253a244f69fb0ddc656f11e1286fb9309a39a92553e2ce3969eeb92ed30bd402a7cbc62ec7d7a4e32f7c125")
        
        let sigMsg = tx.signatureMessage(sighashType: .singleAnyCanPay, inputIndex: 0, scriptCode: prevOuts[0].script.data)
