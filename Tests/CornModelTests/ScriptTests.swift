import XCTest
import CornModel
import BigInt

/// https://en.bitcoin.it/wiki/Script
final class ScriptTests: XCTestCase {
    override class func setUp() {
        eccStart()
    }

    override class func tearDown() {
        eccStop()
    }
    
    func testDrop() {
        let bigNummber = BigInt(UInt64.max) * 2
        let numberData = bigNummber.serialize()
        let script = Script(ops: [.pushBytes(numberData), .drop])
        var stack = [Data]()
        let dummyTx = Tx(version: .v1, ins: [], outs: [], witnessData: [], lockTime: 0)
        let result = script.run(stack: &stack, transaction: dummyTx, prevOuts: [], inputIndex: -1)
        XCTAssert(result)
        XCTAssert(stack.isEmpty)
    }

    func testDup() {
        let bigNummber = BigInt(UInt64.max) * 2
        let numberData = bigNummber.serialize()
        let script = Script(ops: [.pushBytes(numberData), .dup])
        var stack = [Data]()
        let dummyTx = Tx(version: .v1, ins: [], outs: [], witnessData: [], lockTime: 0)
        let result = script.run(stack: &stack, transaction: dummyTx, prevOuts: [], inputIndex: -1)
        XCTAssert(result)
        XCTAssertEqual(stack, [numberData, numberData])
    }

    func testCheckSig() {
        let pubKey = Data(hex: "04ce88102d2af294198df851e4776e4c505e2f288cb253a244f69fb0ddc656f11e1286fb9309a39a92553e2ce3969eeb92ed30bd402a7cbc62ec7d7a4e32f7c125")
        let prevOut = Tx.Out(value: UInt64(0), scriptPubKey: .init(Data(hex: "76a914786890276a55f3e6d2f403e3d595b6603964fa0d88ac"), includeLength: false))

        let tx = Tx(Data(hex: ""))

        let sigOp = tx.ins[0].scriptSig.ops[0]
        guard case .pushBytes(let signature) = sigOp else {
            XCTFail("Could not extract signature from transaction.")
            return
        }

        let pubKeyHash = hash160(pubKey)

        let script = Script(ops: [.pushBytes(signature), .pushBytes(pubKey), .dup, .hash160, .pushBytes(pubKeyHash), .equalVerify, .checkSig])

        var stack = [Data]()
        let result = script.run(stack: &stack, transaction: tx, prevOuts: [prevOut], inputIndex: 0)
        XCTAssert(result)
        XCTAssertEqual(stack, [BigInt(1).serialize()])        
    }
}
