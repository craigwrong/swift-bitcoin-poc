...
final class TapscriptPlaygroundTests: XCTestCase {
    ...
    func testTapscriptSpend() {
        ...
        let prevOuts = [Transaction.Output(value: 100, scriptPubKey: makeP2TR(outputKey: outputKey))]
        
        var tx = Transaction(version: .v1, locktime: .disabled,
            inputs: [.init(txID: "", outIdx: 0, sequence: .initial)],
            outputs: [.init(value: 50, scriptPubKey: makeNullData(""))])
    ...
    func testVectors() {
        for testCase in coreTestAssets {
            ...
            var tx = unsigned
            tx.inputs[inIdx].script = [Op](Data(hex: testCase.success.scriptSig))
            ...
            if let failure = testCase.failure {
                var failTx = unsigned
                failTx.inputs[inIdx].script = [Op](Data(hex: failure.scriptSig))
